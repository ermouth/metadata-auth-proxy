...
Токен юзера-вездехода для доступа к архивам, например `YWRtaW46YWRtaW4=` для `admin:admin`

### ARCHIVE_PROXIES

Перекрывает настройки серверов по абонентам(зонам)/годам в wb_meta.
Если зона прописана в env-переменной, архивный запрос передаётся в указанный прокси, 
при этом Year в заголовках запроса не сбрасывается. 

Пример: `ARCHIVE_PROXIES=10@http://zone.wb.addr,21@http://another.zone.wb`

### ARCHIVE_PROXY_THIS_YEAR

Проксировать запросы текущего года в том числе. По умолчанию `false`, запросы текущего года
